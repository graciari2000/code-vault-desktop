...
async function testServerConnection(serverUrl) {
    try {
        ...
        vscode.window.showErrorMessage(`Code Vault: Cannot connect to server at ${serverUrl}. Make sure the desktop app is running.`);
        return false;
    }
}
async function checkExtensionStatus(serverUrl) {
    ...
    vscode.window.showInformationMessage(`Code Vault Status: ${status.extensionActive ? 'Active' : 'Inactive'}, ` +
...
async function saveSnippetToVault(code, language, title, serverUrl, description) {
    try {
        ...
        const response = await axios_1.default.post(`${serverUrl}/api/snippets`, snippet, { timeout: 10000 });
        ...
        vscode.window.showErrorMessage(`Failed to save code to Code Vault. Make sure the desktop app is running at ${serverUrl}`);
    }
}
