...
function activate(context) {
    ...
    const saveDisposable = vscode.workspace.onDidSaveTextDocument(async (document) => {
        if (!autoCapture || !captureOnSave || !isExtensionActive)
            return;
        const editor = vscode.window.activeTextEditor;
    ...
    context.subscriptions.push(showSnippetsCommand, captureCodeCommand, statusCommand, refreshCacheCommand, editorChangeDisposable, saveDisposable);
...
async function captureSelectionOnly(editor, serverUrl) {
    try {
...
async function captureEntireFile(editor, serverUrl) {
    try {
        const document = editor.document;
...
async function processCodeCapture(code, language, title, serverUrl, isFile = false) {
...
async function loadExistingSnippets(serverUrl) {
    try {
        const response = await axios_1.default.get(`${serverUrl}/api/snippets`, { timeout: 10000 });
...
async function testServerConnection(serverUrl) {
    try {
        const response = await axios_1.default.get(`${serverUrl}/api/health`, { timeout: 5000 });
        ...
        vscode.window.showErrorMessage(`Code Vault: Cannot connect to desktop app at ${serverUrl}. ` +
...
async function checkExtensionStatus(serverUrl) {
    ...
    if (serverReachable) {
        vscode.window.showInformationMessage(`✅ Code Vault: Connected to desktop app, ${existingSnippetsCache.length} snippets cached`);
    }
    else {
        vscode.window.showWarningMessage(`❌ Code Vault: Desktop app not found at ${serverUrl}. Make sure the Code Vault app is running.`);
    }
}
async function isCodeAlreadyInVault(code, serverUrl) {
    try {
        ...
        try {
            const response = await axios_1.default.post(`${serverUrl}/api/ai/analyze`, {
                code,
                language: 'auto'
            }, { timeout: 10000 });
            // If server finds very similar code, consider it a duplicate
            const suggestions = response.data.suggestions || [];
            const highlySimilar = suggestions.some((s) => s.confidence > 0.8);
            if (highlySimilar) {
...
async function captureCurrentCode(serverUrl) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
...
async function saveSnippetToVault(code, language, title, serverUrl, description) {
    try {
        ...
        const response = await axios_1.default.post(`${serverUrl}/api/snippets`, snippetData, {
            timeout: 10000
        });
        ...
        if (error.response) {
            ...
            vscode.window.showErrorMessage(`Failed to save code: ${error.response.data.error || 'Server error'}`);
        }
        else if (error.request) {
            // No response received
            vscode.window.showErrorMessage(`Cannot connect to Code Vault desktop app at ${serverUrl}. Make sure it's running.`);
        }
        else {
            // Other error
            vscode.window.showErrorMessage(`Failed to save code: ${error.message}`);
        }
    }
}
function isDuplicateCode(codeOrHash) {
    return recentlyProcessed.has(codeOrHash);
}
async function showRelevantSnippets(serverUrl) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showWarningMessage('No active editor found');
        return;
    ...
    try {
        ...
        const relevantSnippets = snippets.filter(snippet => snippet.language === mapLanguageId(currentLanguage) ||
        ...
        vscode.window.showErrorMessage('Failed to connect to Code Vault. Make sure the desktop app is running.');
    }
}
// Helper functions
...
function isCodeSimilar(code1, code2) {
